...
const IntimacoesView: React.FC = () => {
  ...
  const [intimacaoSelecionada, setIntimacaoSelecionada] = useState<Intimacao | null>(null);
  ...
  const calcularDiasRestantes = (prazo: string) => {
    ...
    const diferenca = Math.ceil((dataPrazo.getTime() - hoje.getTime()) / (1000 * 60 * 60 * 24));
    return diferenca;
  };
  ...
  const intimacoesFiltradas = intimacoes.filter(intimacao => {
    const matchBusca = !filtros.busca ||
      ...
      intimacao.numero_intimacao.toLowerCase().includes(filtros.busca.toLowerCase());

    const matchStatus = !filtros.status || intimacao.status === filtros.status;
    const matchTipo = !filtros.tipo || intimacao.tipo === filtros.tipo;
    const matchUrgencia = !filtros.urgencia || intimacao.urgencia === filtros.urgencia;
    const matchResponsavel = !filtros.responsavel || intimacao.responsavel === filtros.responsavel;

    return matchBusca && matchStatus && matchTipo && matchUrgencia && matchResponsavel;
  });

  const marcarComoCumprida = (id: string) => {
    setIntimacoes(prev =>
      prev.map(int =>
        int.id === id ? { ...int, status: 'cumprida' } : int
      )
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-slate-600 dark:text-slate-400 text-sm">Urgência Alta</p>
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-slate-600 dark:text-slate-400 text-sm">Cumpridas</p>
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-slate-600 dark:text-slate-400 text-sm">Vencendo Hoje</p>
                    ...
                    <div className="flex items-center gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(intimacao.status)}`}>
                            {getStatusText(intimacao.status)}
                          </span>
                          <div className={`flex items-center gap-1 ${getUrgenciaColor(intimacao.urgencia)}`}>
                        ...
                        <div className="flex items-center gap-6 text-sm text-slate-600 dark:text-slate-400">
                          ...
                          <span>Recebida: {formatarData(intimacao.data_recebimento)}</span>
                        </div>
                      </div>

                      {/* Prazo */}
                      <div className="flex-shrink-0 text-right">
                        <p className={`text-sm font-medium ${getPrazoClass(intimacao.prazo_cumprimento, intimacao.status)}`}>
                          {formatarData(intimacao.prazo_cumprimento)}
                        </p>
                        <p className="text-xs text-slate-500">
                          {intimacao.status === 'cumprida' ? 'Cumprida' :
                           intimacao.status === 'vencida' ? 'Vencida' :
                ...
                <Disclosure.Panel className="px-6 pb-6 border-t border-slate-200 dark:border-slate-700">
                  <div className="pt-4 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      <div>
                        ...
                        <div className="space-y-1 text-sm">
                          <p><span className="text-slate-500">Número:</span> {intimacao.numero_intimacao}</p>
                          <p><span className="text-slate-500">Tipo:</span> {intimacao.tipo}</p>
                          <p><span className="text-slate-500">Meio:</span> {intimacao.meio_intimacao}</p>
                          <p><span className="text-slate-500">Origem:</span> {intimacao.origem}</p>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm">{intimacao.responsavel}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 pt-2 border-t border-slate-200 dark:border-slate-600">
                      ...
                      <Button variant="outline-secondary" size="sm" className="flex items-center gap-2">
      ...
      {intimacoesFiltradas.length === 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 p-12 text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400">
            {filtros.busca || filtros.status || filtros.tipo || filtros.urgencia || filtros.responsavel
      ...
      <div className="bg-slate-50 dark:bg-slate-800 rounded-lg p-4 text-sm text-slate-600 dark:text-slate-400">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <p>
            Exibindo {intimacoesFiltradas.length} de {intimacoes.length} intimações
          </p>
